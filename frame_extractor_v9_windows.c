...
void print_usage() {
    printf("\n🎬 === Frame Extractor v9.0 (WINDOWS) ===\n");
    printf("Extract frames with %d threads + FFmpeg audio!\n\n", NUM_SAVER_THREADS);
    ...
    printf("📝 NOTE: Use forward slashes (C:/path/file) or double backslashes (C:\\\\path\\\\file)\n\n");
}
...
int check_ffmpeg() {
    ...
    if (system("ffmpeg -version > nul 2>&1") == 0) {
        printf("   ✓ Found ffmpeg in PATH\n");
        return 1;
    }
    
    if (system("ffmpeg.exe -version > nul 2>&1") == 0) {
        printf("   ✓ Found ffmpeg.exe in current directory\n");
        return 1;
    }
    
    if (system("\"C:\\msys64\\mingw64\\bin\\ffmpeg.exe\" -version > nul 2>&1") == 0) {
        printf("   ✓ Found ffmpeg.exe in MSYS2\n");
        return 1;
    }
    
    printf("❌ FFmpeg not found! Please install FFmpeg\n");
    return 0;
}

const char* get_ffmpeg_path() {
    if (system("ffmpeg -version > nul 2>&1") == 0) return "ffmpeg";
    if (system("ffmpeg.exe -version > nul 2>&1") == 0) return "ffmpeg.exe";
...
void* extract_audio_thread(void* arg) {
    ...
    if (!check_ffmpeg()) return NULL;
    
    const char* ffmpeg_path = get_ffmpeg_path();
    ...
    snprintf(command, sizeof(command),
             "%s -i \"%s\" %s -vn %s -acodec %s -y \"%s\" 2>&1",
             ffmpeg_path, fixed_input, time_option, bitrate_opt, codec, final_output);
...
int main(int argc, char** argv) {
    ...
    strcpy(config.audio_output, "audio.mp3");
    config.step = 1;
    config.format = 0;
    config.fast_mode = 0;
...
if (total_frames <= 0) {
    ...
    // Method 1: Format context duration
    ...
    // Method 3: Ask user for duration (last resort)
    if (duration <= 0) {
        char input[100];
        printf("⌛ Could not detect duration automatically.\n");
        printf("   Please enter video duration in seconds: ");
        fflush(stdout);
    ...
    if (duration > 0) {
        // Calculate frames with ceil to ensure we get all frames
        double exact_frames = duration * fps;
        total_frames = (int)ceil(exact_frames);
        
        printf("   Duration: %.3f seconds\n", duration);
        printf("   FPS: %.2f\n", fps);
        printf("   Calculated frames: %.3f\n", exact_frames);
        printf("   Using: %d total frames (rounded up)\n", total_frames);
    } else {
...
// =============================================
    ...
    printf("\n🔄 Decoding frames with %d threads...\n", NUM_SAVER_THREADS);
        ...
        if (packet.stream_index == video_stream_idx) {
            ...
            while (avcodec_receive_frame(codec_ctx, frame) == 0) {
                if (current_frame >= start_frame && current_frame <= end_frame &&
                    (config.frame_count == 0 || frame_in_list(current_frame, frames_to_extract, extract_count))) {
                    queue_push(&frame_queue, frame, current_frame);
                    frames_queued++;
                    frames_decoded++;
                    if (frames_decoded % 10 == 0) {
                        printf("\r📽️ Decoded: %d/%d", frames_decoded, extract_count);
                        fflush(stdout);
                    }
                }
                current_frame++;
